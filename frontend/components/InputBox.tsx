  ...
  return (
    <div className="flex justify-center px-4 pb-6 pt-2">
      <div className="relative w-full max-w-[600px]">
        {/* Input container */}
        <motion.div
          className="glass relative flex items-end rounded-2xl border border-white/10 p-2 shadow-xl"
        >
          {/* Attachment button */}
          <motion.button
            ...
            whileTap={{ scale: 0.9 }}
            type="button"
            className="mb-1 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl text-slate-500 transition-colors hover:bg-white/5 hover:text-white"
          ...
          <AnimatePresence>
            {tokenCount > 0 && (
              <motion.div
                ...
                className="mb-2 mr-2 text-xs font-mono text-slate-600"
              >
                {tokenCount}
              </motion.div>
            )}
          ...
          <AnimatePresence mode="wait">
            {isLoading ? (
              <motion.button
                ...
                type="button"
                onClick={handleStop}
                className="mb-1 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-red-500 to-pink-600 text-white shadow-lg shadow-red-500/30 transition-all hover:shadow-red-500/50"
              ...
              <motion.button
                ...
                onClick={handleSend}
                disabled={!input.trim()}
                className={`mb-1 flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-xl transition-all disabled:cursor-not-allowed disabled:opacity-30 ${
                  input.trim()
